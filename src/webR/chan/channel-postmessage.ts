...
export class PostMessageChannelMain extends ChannelMain {
  ...
  constructor(config: Required<WebROptions>) {
    ...
    if (WebAssembly.Suspender === 'undefined') {
      console.warn(
        'Warning: webR is using the PostMessage channel without WebAssembly.Suspender() support',
        'Some webR features will be unavailable. Unless unable, we recommend switching to using',
        'using webR with the SharedArrayBuffer channel.'
      );
    }
